...
test("set selected piece unselects if current is selected", () => {
    ...
    const newState = state.selectPiece({ row: 1, column: 2 });
    const noSelectedState = newState.selectPiece({ row: 1, column: 2 });
...
test("set selected happy path", () => {
    ...
    const newState = state.selectPiece( { row: 1, column: 7 });
